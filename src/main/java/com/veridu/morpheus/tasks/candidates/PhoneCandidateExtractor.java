 ...
 */
@Component("phone-candidates")
public class PhoneCandidateExtractor implements ITask {
    ...
    private static final IFact candidateFact = new Fact("bestCandidatePhone", "skynet");
    ...
    public void runTask(@RequestBody Parameters params) {
        ...
        HashMap<IFact, String> userFacts = dataSource.obtainSpecificFactForUser(factory, user, "*phoneNumber*");
        ...
        if (candidates.size() > 0) {
            dataSource.insertAttributeCandidatesForUser(factory, user, "phone", candidates);
            //       if (verbose)
            logger.info(
                    String.format("Phone candidate extractor found best candidate: %s with support %.2f for user %s",
                            candidates.get(0).getValue(), candidates.get(0).getSupportScore(), userId));
        } else if (verbose)
            logger.info(String.format("Phone candidate extractor found no candidates for user %s", userId));
