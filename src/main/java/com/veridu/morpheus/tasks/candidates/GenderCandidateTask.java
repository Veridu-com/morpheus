 ...
 */
@Component("gender-candidates")
public class GenderCandidateTask implements ITask {
    ...
    private static final IFact candidateFact = new Fact("bestCandidateGender", "skynet");
    ...
    public void runTask(@RequestBody Parameters params) {
        ...
        HashMap<IFact, String> userFacts = dataSource.obtainSpecificFactForUser(factory, user, "*gender*");
        ...
        if (candidates.size() > 0) {
            dataSource.insertAttributeCandidatesForUser(factory, user, "gender", candidates);
            //   if (verbose)
            logger.info(
                    String.format("Gender candidate extractor found best candidate: %s with support %.2f for user %s",
                            candidates.get(0).getValue(), candidates.get(0).getSupportScore(), userId));
        } else if (verbose)
            logger.info(String.format("Gender candidate extractor found no candidates for user %s", userId));
