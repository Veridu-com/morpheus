 ...
 */
@Component("genderExtractor")
public class BeanGenderFeatureExtractor implements IFeatureExtractor {
     ...
     */
    @Autowired
    public BeanGenderFeatureExtractor(IDataSource dataSource, IUtils utils, IMongoDataSource mongo,
    ...
    private static final String[] providersProfileGender = { Constants.CPR_PROVIDER_NAME,
            Constants.FACEBOOK_PROVIDER_NAME, Constants.GOOGLE_PROVIDER_NAME, Constants.PERSONAL_PROVIDER_NAME,
            Constants.YAHOO_PROVIDER_NAME };

    private static final String[] providersNameGender = { Constants.AMAZON_PROVIDER_NAME, Constants.CHECK_PROVIDER_NAME,
            Constants.CPR_PROVIDER_NAME, Constants.DROPBOX_PROVIDER_NAME, Constants.FACEBOOK_PROVIDER_NAME,
            Constants.GOOGLE_PROVIDER_NAME, Constants.LINKEDIN_PROVIDER_NAME, Constants.PAYPAL_PROVIDER_NAME,
            Constants.SPOTIFY_PROVIDER_NAME, Constants.TWITTER_PROVIDER_NAME, Constants.YAHOO_PROVIDER_NAME };

    private static final ArrayList<IFact> providerProfileGenderFacts = LocalUtils
            .generateProvidersFact(providersProfileGender, "profileGender");

    private static final ArrayList<IFact> providerNameGenderFacts = LocalUtils
            .generateProvidersFact(providersNameGender, "nameGender");

    private static ArrayList<IFact> providerFacts;

    private static final String myProviderName = "gender-features";

    private static RulesEngine rulesEngineProfileGender;

    private static RulesEngine rulesEngineNameGender;
    ...
    private void init() {
        ...
        providerFacts.addAll(providerProfileGenderFacts);
        providerFacts.addAll(providerNameGenderFacts);

        rulesEngineProfileGender = new RulesEngine("profileGender", myProviderName, providersProfileGender, false, 0);
        rulesEngineNameGender = new RulesEngine("nameGender", myProviderName, providersProfileGender, false, 0);
        ...
        this.factList.addAll(rulesEngineProfileGender.getFactList());
        this.factList.addAll(rulesEngineNameGender.getFactList());
    ...
    /**
     * Create an instance with gender features
     *
     * @param factory idOS API factory
     * @param dataset data header
     * @param user selected user
     * @return an Instance object with the features regarding gender
     */
    @Override
    public Instance createInstance(IdOSAPIFactory factory, Instances dataset, IUser user) {
        ...
        // get provider facts
        HashMap<IFact, String> userFactsProfileGender = dataSource
                .obtainSpecificFactForUser(factory, user, "profileGender");
        ArrayList<String> profileGenderFactValues = new ArrayList<>();

        for (IFact provFact : providerProfileGenderFacts)
            if (userFactsProfileGender.containsKey(provFact))
                profileGenderFactValues.add(userFactsProfileGender.get(provFact).toLowerCase());
            else
                profileGenderFactValues.add("");

        HashMap<IFact, String> userFactsNameGender = dataSource.obtainSpecificFactForUser(factory, user, "nameGender");
        ArrayList<String> nameGenderFactValues = new ArrayList<>();

        for (IFact provFact : providerNameGenderFacts)
            if (userFactsNameGender.containsKey(provFact))
                nameGenderFactValues.add(userFactsNameGender.get(provFact).toLowerCase());
            else
                nameGenderFactValues.add("");

        // rules
        double[] values = rulesEngineProfileGender.applyRules(profileGenderFactValues);
        for (int i = 0; i < values.length; i++)
            inst.setValue(attPosCounter++, values[i]);

        values = rulesEngineNameGender.applyRules(nameGenderFactValues);
        for (int i = 0; i < values.length; i++)
            inst.setValue(attPosCounter++, values[i]);
        ...
        if (user instanceof IFakeUsUser) {
            ...
            ArrayList<ICandidate> candidates = fuser.getAttributesMap().get(new Attribute("gender"));
        ...
        if (DEBUG) {
            System.out.println("--------------------------");
            System.out.println("Gender feature extractor for user " + user.getId());
            System.out.println("Profile gender providers array:");
            System.out.println(Arrays.toString(providersProfileGender));
            System.out.println("Gender list (follows order of providers):");
            System.out.println(Arrays.toString(profileGenderFactValues.toArray()));
            System.out.println("Name gender providers array:");
            System.out.println(Arrays.toString(providersNameGender));
            System.out.println("Gender list (follows order of providers):");
            System.out.println(Arrays.toString(nameGenderFactValues.toArray()));
