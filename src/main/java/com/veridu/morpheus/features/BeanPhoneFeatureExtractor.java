...
public class BeanPhoneFeatureExtractor implements IFeatureExtractor {
     ...
     */
    @Autowired
    public BeanPhoneFeatureExtractor(IDataSource dataSource, IUtils utils, IMongoDataSource mongo,
    ...
    private static final String[] providers = { Constants.PAYPAL_PROVIDER_NAME, Constants.PERSONAL_PROVIDER_NAME,
            Constants.SMS_PROVIDER_NAME };

    private static final String phoneFactName = "phoneNumber";

    private static final ArrayList<IFact> providerFacts = LocalUtils.generateProvidersFact(providers, phoneFactName);

    private static final String myProviderName = "phone-features";

    private static RulesEngine rulesEngine = new RulesEngine(phoneFactName, myProviderName, providers, false, 0.6);
    ...
    /**
     * Create an instance with phone features
     *
     * @param factory idOS API factory
     * @param dataset data header
     * @param user selected user
     * @return an Instance object with the features regarding phone
     */
    @Override
    public Instance createInstance(IdOSAPIFactory factory, Instances dataset, IUser user) {
        ...
        // get provider facts
        HashMap<IFact, String> userFacts = dataSource.obtainSpecificFactForUser(factory, user, phoneFactName);
        ...
        if (user instanceof IFakeUsUser) {
            ...
            ArrayList<ICandidate> candidates = fuser.getAttributesMap().get(new Attribute("phone"));
        ...
        if (DEBUG) {
            System.out.println("--------------------------");
            System.out.println("Phone feature extractor for user " + user.getId());
            System.out.println("Providers array:");
            System.out.println(Arrays.toString(providers));
            System.out.println("Phone list (follows order of providers):");
