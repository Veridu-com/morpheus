 ...
 */
@Component("countryExtractor")
@Scope("singleton")
public class BeanCountryFeatureExtractor implements IFeatureExtractor {
    ...
    private static final String countryFactName = "countryName";

    private static final ArrayList<IFact> providerFacts = LocalUtils.generateProvidersFact(providers, countryFactName);

    private static final String myProviderName = "country-features";

    private static RulesEngine rulesEngine = new RulesEngine(countryFactName, myProviderName, providers, false, 0.6);
    ...
    public Instance createInstance(IdOSAPIFactory factory, Instances dataset, IUser user) {
        ...
        if (user instanceof IFakeUsUser) {
            ...
            ArrayList<ICandidate> candidates = fuser.getAttributesMap().get(new Attribute("country"));
        ...
        if (DEBUG) {
            System.out.println("--------------------------");
            System.out.println("Country feature extractor for user " + user.getId());
            System.out.println("Providers array:");
            System.out.println(Arrays.toString(providers));
            System.out.println("Country list (follows order of providers):");
