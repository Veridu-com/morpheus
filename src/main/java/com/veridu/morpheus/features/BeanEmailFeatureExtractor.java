...
public class BeanEmailFeatureExtractor implements IFeatureExtractor {
    ...
    private static final String[] providers = { Constants.AMAZON_PROVIDER_NAME, Constants.DROPBOX_PROVIDER_NAME,
            Constants.FACEBOOK_PROVIDER_NAME, Constants.GOOGLE_PROVIDER_NAME, Constants.LINKEDIN_PROVIDER_NAME,
            Constants.PAYPAL_PROVIDER_NAME, Constants.PERSONAL_PROVIDER_NAME, Constants.SPOTIFY_PROVIDER_NAME };

    private static final String emailFactName = "emailAddress";

    private static final ArrayList<IFact> providerFacts = LocalUtils.generateProvidersFact(providers, emailFactName);

    private static final String myProviderName = "email-features";

    private static RulesEngine rulesEngine = new RulesEngine(emailFactName, myProviderName, providers, false);
    ...
    public Instance createInstance(IdOSAPIFactory factory, Instances dataset, IUser user) {
        ...
        // get provider facts
        HashMap<IFact, String> userFacts = dataSource.obtainSpecificFactForUser(factory, user, emailFactName);
        ...
        if (user instanceof IFakeUsUser) {
            ...
            ArrayList<ICandidate> candidates = fuser.getAttributesMap().get(new Attribute("email"));
        ...
        if (DEBUG) {
            System.out.println("--------------------------");
            System.out.println("Email feature extractor for user " + user.getId());
            System.out.println("Providers array:");
            System.out.println(Arrays.toString(providers));
            System.out.println("Email list (follows order of providers):");
